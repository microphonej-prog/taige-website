#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""2026-09-25 下午批次生成前体检：两篇正文文件的六语属性完整性 / 引号嵌套 / 未转义 & / 非中文语种混入汉字"""
import re, sys
from html.parser import HTMLParser

BODIES = ["blog/_body_footwear.html", "blog/_body_ip.html"]
...
for b in BODIES:
    ...
    zh_text = "".join(p.text)
    print("  正文中文字数（默认文本节点）：%d" % len(CJK.findall(zh_text)))
    # 正文中文字数（按 data-zh 统计，更贴近实际）
    zh_attr = "".join(re.findall(r'data-zh="([^"]*)"', src))
    print("  data-zh 中文字数：%d" % len(CJK.findall(zh_attr)))
