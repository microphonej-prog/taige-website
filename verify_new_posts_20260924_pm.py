#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""2026-09-24 下午批次校验：六语文件齐备 / canonical+hreflang / JSON-LD / 六语覆盖 /
   列表页卡片顺序 / sitemap URL / 语言页无中文残留 / 中文根页静态 description 与 title"""
...
for slug in SLUGS:
    ...
    # 0) 中文根页：<title> 静态内容 = data-zh；description 有 content 且 = data-zh 值
    tm = re.search(r'<title data-zh="([^"]*)"[^>]*>([^<]*)</title>', s, re.S)
    if not tm or tm.group(1) != tm.group(2):
        problems.append("%s 中文 title 静态内容与 data-zh 不一致" % root)
    dm = re.search(r'<meta name="description" data-zh="([^"]*)"(.*?)>', s, re.S)
    if not dm:
        problems.append("%s description 结构异常" % root)
    else:
        cm = re.search(r'content="([^"]*)"', dm.group(2))
        if not cm or cm.group(1) != dm.group(1):
            problems.append("%s 中文 description content 与 data-zh 不一致" % root)
        else:
            print("  中文静态 description %d 字 ✓" % len(cm.group(1)))

    # 1) canonical + hreflang
    m = re.search(r'<link rel="canonical" href="([^"]+)">', s)
    if not m or m.group(1) != "https://taigetag.com/blog/%s" % slug:
        problems.append("canonical 错: %s" % (m.group(1) if m else "缺失"))
    hrefl = re.findall(r'<link rel="alternate" hreflang="([^"]+)" href="([^"]+)">', s)
    if len(hrefl) != 7:
        problems.append("hreflang 数量 %d（应为 7）" % len(hrefl))
    print("  canonical=%s hreflang=%d" % (m.group(1) if m else "-", len(hrefl)))
    ...
    # 4) 语言页：无 data-* 残留、标题语言正确、desc 不短
    for l in LANGS:
        p = "%s/blog/%s" % (l, slug)
        t = open(p, encoding="utf-8").read()
        ...
        if l == "ko" and re.search(r'[\u4e00-\u9fff]', title):
            problems.append("%s 韩语标题含汉字: %s" % (p, title))
...
if bv != "106":
    problems.append("BUST_VERSION = %s（应为 106）" % bv)
