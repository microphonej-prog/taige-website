#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""校验 2026-09-22 下午新增的 2 篇六语文章：正文六语完整性、引号畸形、JSON-LD、
生成页残留与语言真实渲染、hreflang/canonical、静态 title/description、sitemap、
列表页卡片、BUST_VERSION。"""
import re, json, os, sys
from html.parser import HTMLParser

NEW = ["blog/garment-trims-order-tracking-guide.html", "blog/hang-tag-ink-safety-guide.html"]
...
for f in NEW:
    ...
    print("  %-46s lastmod=%s" % (slug, z.group(1) if z else "缺失"))
...
for p in ["blog/index.html"] + ["%s/blog/index.html" % l for l in LANGS]:
    ...
    print("  %-46s lastmod=%s" % (p, z.group(1) if z else "缺失"))
    if not z or z.group(1) != DATE:
