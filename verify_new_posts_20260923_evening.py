#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""2026-09-23 晚间批次校验：六语文件齐备 / canonical+hreflang / JSON-LD / 六语覆盖 /
   列表页卡片 / sitemap URL / 语言页无中文残留（en/fr/es）/ 版本号"""
...
for slug in SLUGS:
    ...
    # 2) 根页正文六语覆盖
    miss_fr = miss_es = miss_ja = miss_ko = miss_en = n_data = 0
    ...
    for t in split_tags(body):
        if re.match(r'<(p|h2|h3|li|td|th|div|a|span)\b', t) and 'data-zh=' in t:
            n_data += 1
            miss_en += ' data-en=' not in t
            miss_fr += ' data-fr=' not in t
            miss_es += ' data-es=' not in t
            miss_ja += ' data-ja=' not in t
            miss_ko += ' data-ko=' not in t
    print("  正文元素 %d，缺 en=%d fr=%d es=%d ja=%d ko=%d"
          % (n_data, miss_en, miss_fr, miss_es, miss_ja, miss_ko))
    if miss_en or miss_fr or miss_es or miss_ja or miss_ko:
        problems.append("%s 正文六语缺失 en=%d fr=%d es=%d ja=%d ko=%d"
                        % (slug, miss_en, miss_fr, miss_es, miss_ja, miss_ko))
    ...
    # 4) 语言页：无 data-* 残留、标题语言正确、desc 长度
    for l in LANGS:
        p = "%s/blog/%s" % (l, slug)
        t = open(p, encoding="utf-8").read()
        ...
        print("  [%s] %s | desc %d" % (l, title[:68], len(desc)))
...
bv = re.search(r'var BUST_VERSION = "(\d+)";', mj).group(1)
print("BUST_VERSION = %s" % bv)
if bv != "104":
    problems.append("BUST_VERSION = %s（应为 104）" % bv)

# 9) 卡片顺序与旧卡片未被覆盖
idx = open("blog/index.html", encoding="utf-8").read()
print("blog/index.html 卡片总数 = %d" % idx.count('class="post-card"'))
