#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""2026-09-25 下午批次本地验证：文件/卡片/sitemap/六语属性/链接/繁体/版本号"""
import os, re, sys, json, html, importlib.util
...
for l in ALL:
    p = ("blog/index.html" if not l else "%s/blog/index.html" % l)
    ...
    ck("② %s 语言列表页顶部两张卡正确" % (l or "zh"), sorted(top2) == sorted(SLUGS), str(top2))
for slug in SLUGS:
...
for slug in SLUGS:
    ...
    for a in elems:
        ...
        if lack:
            missing.append(lack)
    ck("⑤ %s article-body 缺语言 = 0" % slug, not missing,
...
ck("⑧ BUST_VERSION = 109", m and m.group(1) == "109", m.group(1) if m else "none")
    ...
    for l in ALL:
        ...
        if ok:
            try:
                ...
                ok = False
        ck("⑨ JSON-LD 合法 %s" % p, ok, "%d 块" % len(blocks))
