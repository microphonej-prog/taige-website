#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""校验 2026-09-21 晚间新增的 2 篇六语文章：正文六语完整性、引号畸形、JSON-LD、
...
for f in NEW:
    ...
    for lang in LANGS:
        ...
        if lang == "ko" and hangul < 50:
            ok = False
        print("  %-46s residue=%d 汉字=%d 假名=%d 韩文=%d desc=%d hreflang=%d %s"
              % (lang + "/blog/" + slug.replace("clothing-label-compliance-", "c-l-c-"), residue,
                 cjk, kana, hangul, len(desc.group(1)) if desc else -1, hl, "OK" if ok else "!! 异常"))
        print("        title: %s" % title[:92])
        if not ok:
...
for p in ["blog/index.html"] + ["%s/blog/index.html" % l for l in LANGS]:
    ...
    if not z or z.group(1) != "2026-09-21":
